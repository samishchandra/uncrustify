...
// R formatted strings
NSString *expected = @R"({"mapping":"%"})";

auto expected = @R"({'mapping':'%'})";

auto groupID = [NSString stringWithFormat:@R"({"group_id ":"%@",})", group.graphQLID];

NSString *config = @R"({"config_v2":[
  {"optimization":{"type":{"unsupported1":{}}},"markers":[123]},
  {"optimization":{"type":{"appJobs":{"suspend":true,"resume":false}}},"markers":[1]},
  {"optimization":{"type":{"Unsupported Optimization With Good Format":{}}},"markers":[123]}
  ]})";

NSString *mfStoryKey = _ConversionIdentifierStringWithPattern(
  trackingString,
  @R"(\"mf_story_key\":\"(\d*)\")"
);

// functions after macros
typedef std::vector<string *> *(*Finder)(std::string *);
typedef vector<std::string *> *(*Handler)(std::map<std::string *, vector *> *);

- (void)match:(Sender)obj
      message:(HandlerParam)handler NS_SWIFT_NAME(match(first: second:));

NS_SWIFT_NAME(SOMEOTHER(EVERY));
Controller *create(NSString *str);

FB_LINKABLE(ThreadButton)
UIViewController *MSGCreate(MBAMailbox *mailbox, NSNumber *threadKey);

FB_LINKABLE(Button)
UIViewController *MSGCreate2(MBAMailbox *mailbox,
                             NSNumber *threadKey);

// Pointer handling inside OC blocks inside typedef
typedef NSArray<NSString *> *(^Finder)(void);
...
_sections2 = @[
  [*[SectionModel mesg] resultsWithContacts:contacts]
];
...
[**messageClass2 paramName2:argValue2];

// Modernized raw strings
NSString *predicate = [NSString stringWithFormat:@R"Something"];
NSString *variable = @R"Everything"
...
#import <KSCDefines/KSCDefines.h>
#import <ModuleA/FileA.h>

#import "ABC.h"
#include "AFile.h"
#import "FDSBottomSheet.h"
#import "FDSBottomSheet.hpp"
...
[ViewController preFirst:(
 ...
 arg1
       ...
       selector_number_5:getArgForSelector(
   flag
   ? arg5_1
   : arg5_2
       )
...
// OC property sorting
@property (class, nonnull, nonatomic, readonly, getter = test5Getter, assign) NSString *test5;
@property (class, getter = test5Getter, assign, myatomic, nonnull_strong) NSString *test6;
@property (null_unspecified, nonatomic, readonly, strong) NSString *test1;
